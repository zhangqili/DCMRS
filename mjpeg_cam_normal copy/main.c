...
static uint8_t processedpicture[CROP_WQVGA_X * CROP_WQVGA_Y * CAM_BUFF_NUM/2] ATTR_NOINIT_PSRAM_SECTION __attribute__((aligned(64)));
...
int main(void)
{
    ...
    cam_config.input_source = CAM_INPUT_SOURCE_DVP;
    cam_config.output_format = CAM_OUTPUT_FORMAT_AUTO;
    cam_config.output_bufaddr = (uint32_t)BFLB_PSRAM_BASE; //0XA8000000
    ...
    config.input_bufaddr0 = (uint32_t)BFLB_PSRAM_BASE;
    config.input_bufaddr1 = 0;
    config.output_bufaddr = (uint32_t)BFLB_PSRAM_BASE + CROP_WQVGA_X * 2 * ROW_NUM;
    config.output_bufsize = SIZE_BUFFER + CROP_WQVGA_X * 2 * ROW_NUM;
    config.input_yy_table = NULL; // use default table
    config.input_uv_table = NULL; // use default table
    ...
    //     bflb_mtimer_delay_ms(200);
    // }
    uint8_t *pic_now = (uint8_t *)processedpicture;
    int jpeg_len;
    printf("base:\t%u\n",BFLB_PSRAM_BASE);
    printf("pic:\t%u\n",pic);
    printf("picture:\t%u\n",picture);
    ...
    while (1) {
        ...
        if(bflb_mjpeg_get_frame_count(mjpeg)>0)
        {
            ...
            // printf("%d :%d\r\n",i,pic_now[i]);
            yuyv_to_rgb(pic,processedpicture,CROP_WQVGA_X,ROW_NUM);
            //bflb_mtimer_delay_ms(200);
        printf("ready to boom\n");
            //img_src_cam.data = pic_now;
        printf("boo\n");
            //lv_img_set_src(avatar, LV_SYMBOL_OK);
        }
        */
#else
        uint32_t intstatus = bflb_mjpeg_get_intstatus(mjpeg);//表示中断状态
        if (intstatus & MJPEG_INTSTS_ONE_FRAME) {
            bflb_mjpeg_get_frame_info(mjpeg, &pic);
            pic_addr[pic_count] = (uint32_t)pic;
            pic_len[pic_count] = jpeg_len;
            pic_count++;
            if(pic_count>5)pic_count=0;
