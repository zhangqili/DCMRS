...
volatile uint32_t pic_addr[999] = { 0 };
volatile uint32_t pic_len[999] = { 0 };
...
//     }
    ...
    sprintf(file_name, "/sd/photo_%d.jpg", pic_count);
    memcpy(&RW_Buffer, (uint8_t *)pic_addr[pic_count-1], pic_len[pic_count-1]);

    LOG_F("\r\n  size of pic %d, size of rw_buffer %d", pic_len[pic_count-1], sizeof(RW_Buffer));
